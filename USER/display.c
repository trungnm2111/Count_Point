...
{  
	...
	int i;
	num = num - ' '; // lay gia tri so tuong ung voi ki tu
	
	if(size <= 16)
	{
		LCD_SetWindows(x, y, x + size/2 - 1, y + size - 1); // thiet lap vung hien thi cho 1 ki tu
		if(!mode)  // che do khong chong lan
		{		
			for(pos = 0; pos < size; pos++)
			{
				...
				{                 
					if(temp&0x01)
						Lcd_WriteData_16Bit(fc); // ve diem mau chu
					else 
						Lcd_WriteData_16Bit(bc); // ve diem mau nen
					temp>>=1; 
				}
			
			}	
		}
		else  // che do chong lan
		{
			for(pos = 0; pos < size; pos++)
			{
				if(size==12)
				...
				{   
					POINT_COLOR=fc;              
					if(temp&0x01)
						LCD_DrawPoint(x + t,y + pos); // ve tung diem mau chu
					temp>>=1; 
				}
			}
		}   	   	 	  
...
***/
void LCD_Show_24x48_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode)
{
	u32 temp;
	u8 pos,t;
	u16 colortemp=POINT_COLOR;      		   
...
void LCD_ShowString_24x48(u16 x, u16 y, u16 fc, u16 bc, u8 *p, u8 mode)
{         
    while((*p <= '~')&&(*p >= ' ')) 
    {   
		if(x > (lcddev.width-1)|| y > (lcddev.height-1)) 
		return;
		POINT_COLOR = GRAYBLUE;
        LCD_Show_24x48_char(x, y, fc, bc, *p, mode);
        x += 48/2 + 1;
        p++;
    }  
} 
...
/**
	* function: TFT_Show_24x48_char - display character 48 size 
***/
//void LCD_Show_108x144_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode)
//{
//	u32 temp;
//	u8 pos,t;
//	u16 colortemp=POINT_COLOR;      		   
//	num=num-' ';//Get the offset value
//	
//	LCD_SetWindows(x, y, x + 108-1, y + 144-1);//Set a single text display window
//	
//	if(!mode)//Non-overlay mode: The font has a background color, which will overwrite the original display	
//	{ 	
//		for(pos = 0; pos < 144; pos++)
//		{ 
//			temp = (Number[num][pos*3]<<16)|(Number[num][pos*3+1]<<8)|Number[num][pos*3+2];//Call 4824 font, you need to take the model definition
//			for(t=0;t<108;t++)
//			{                 
//				if(temp&0x800000){
//					Lcd_WriteData_16Bit(fc);
